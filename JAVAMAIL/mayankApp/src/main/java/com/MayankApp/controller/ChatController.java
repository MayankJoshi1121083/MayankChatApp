...
import com.MayankApp.entity.*;
import com.MayankApp.repository.ChatMessageRepository;
import com.MayankApp.repository.ChatRoomRepository;
import com.MayankApp.repository.UserRepository;
import com.MayankApp.service.ChatRoomService;
...
public class ChatController {
    ...
    @MessageMapping("user/chat/{chatId}")
    public void sendPrivateMessage(@DestinationVariable String chatId, Greeting message, Principal principal) {
        ...
        ChatMessage chatMessage = ChatMessage.builder().id(UUID.randomUUID().toString()).message(message.getMessage())
                .senderId(senderId).recipientId(recipientId).timestamp(timestamp).chatId(chatId).build();


        // Send the private message to the recipient's user-specific destination

        messagingTemplate.convertAndSendToUser(recipientId, "queue/mess/" + chatId,
                message.getMessage());
    }
